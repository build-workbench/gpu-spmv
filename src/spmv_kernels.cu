...
static int prepare_texture_context(SpMVExecutionContext* context,
                                   const float* d_x, size_t x_length,
  ...
  *use_texture_out = false;
  ...
  if (needs_rebuild) {
    ...
    cudaError_t err =
        cudaCreateTextureObject(&context->tex_x, &res_desc, &tex_desc, nullptr);
    if (err != cudaSuccess) {
      context->reset();
      return static_cast<int>(SpMVError::CUDA_MALLOC);
    }
...
// Merge Path 搜索
__device__ MergeCoordinate merge_path_search(int diagonal, const int* row_ptrs,
...
__global__ void spmv_csr_merge_path_kernel(int num_rows, int nnz,
  ...
  MergeCoordinate start =
      merge_path_search(diagonal_start, row_ptrs, num_rows, nnz);
  MergeCoordinate end =
      merge_path_search(diagonal_end, row_ptrs, num_rows, nnz);
  ...
  while (current_row < end.row ||
         (current_row == end.row && current_nz < end.nz)) {
    if (current_row < num_rows) {
      int row_end = row_ptrs[current_row + 1];

      while (current_nz < row_end &&
             (current_row < end.row || current_nz < end.nz)) {
        sum += values[current_nz] *
               fetch_x(x, tex_x, use_texture, col_indices[current_nz]);
        current_nz++;
      }
...
__global__ void spmv_csr_vector_kernel(int num_rows, const int* row_ptrs,
                                       const int* col_indices,
                                       const float* values, const float* x,
                                       cudaTextureObject_t tex_x,
                                       bool use_texture, float* y) {
...
__global__ void spmv_csr_scalar_kernel(int num_rows, const int* row_ptrs,
                                       const int* col_indices,
                                       const float* values, const float* x,
                                       cudaTextureObject_t tex_x,
                                       bool use_texture, float* y) {
...
SpMVResult spmv_csr(const CSRMatrix* A, const float* d_x, float* d_y,
  ...
  if (use_texture && texture_length > 0) {
    if (context) {
      int tex_status = prepare_texture_context(
          context, d_x, texture_length, use_texture, &tex_x, &use_texture);
  ...
  switch (config->kernel_type) {
    case SpMVConfig::MERGE_PATH: {
      ...
      spmv_csr_merge_path_kernel<<<num_blocks, block_size>>>(
          A->num_rows, A->nnz, A->d_row_ptrs, A->d_col_indices, A->d_values,
    ...
    case SpMVConfig::VECTOR_CSR: {
      ...
      spmv_csr_vector_kernel<<<num_warps, block_size>>>(
          A->num_rows, A->d_row_ptrs, A->d_col_indices, A->d_values, d_x, tex_x,
          use_texture, d_y);
      break;
    }
    case SpMVConfig::SCALAR_CSR:
  ...
  result.bandwidth_gb_s =
      compute_bandwidth_csr(A, result.elapsed_ms).achieved_bandwidth_gb_s;
  result.error_code = static_cast<int>(SpMVError::SUCCESS);
...
SpMVResult spmv_ell(const ELLMatrix* A, const float* d_x, float* d_y,
  ...
  if (config->kernel_type != SpMVConfig::ELL_KERNEL ||
      !is_valid_block_size(config->block_size)) {
    result.error_code = static_cast<int>(SpMVError::INVALID_ARGUMENT);
    return result;
  }
  ...
  result.gflops = (2.0f * A->nnz) / (result.elapsed_ms * 1e6f);
  result.bandwidth_gb_s =
      compute_bandwidth_ell(A, result.elapsed_ms).achieved_bandwidth_gb_s;
  result.error_code = static_cast<int>(SpMVError::SUCCESS);
