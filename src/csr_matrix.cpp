...
int csr_to_dense(const CSRMatrix* csr, float* dense) {
  ...
  // Check for potential overflow in size calculation
  size_t total_size =
      static_cast<size_t>(csr->num_rows) * static_cast<size_t>(csr->num_cols);
...
int csr_to_gpu(CSRMatrix* mat) {
  ...
  if (mat->nnz > 0) {
    err = cudaMemcpy(new_d_values, mat->values, mat->nnz * sizeof(float),
...
int csr_serialize(const CSRMatrix* mat, const char* filename) {
  ...
  file.write(reinterpret_cast<const char*>(mat->row_ptrs),
             (mat->num_rows + 1) * sizeof(int));
...
int csr_deserialize(CSRMatrix* mat, const char* filename) {
  ...
  if (version > 1) {
    fprintf(stderr,
            "Warning: CSR file version %u is newer than supported version 1\n",
            version);
  }
...
CSRStats csr_compute_stats(const CSRMatrix* mat) {
  ...
  stats.skewness =
      static_cast<float>(stats.max_nnz_per_row) / (stats.min_nnz_per_row + 1);
