...
__global__ void compute_l2_diff_kernel(const float* a, const float* b,
...
static std::vector<int> find_dangling_nodes(const CSRMatrix* adj_matrix) {
  ...
  if (!adj_matrix->values || !adj_matrix->col_indices ||
      !adj_matrix->row_ptrs) {
    return dangling;
  }
...
PageRankResult pagerank(const CSRMatrix* adj_matrix,
  ...
  if (adj_matrix->num_rows < 0 || adj_matrix->num_cols < 0 ||
      adj_matrix->nnz < 0) {
    result.error_code = static_cast<int>(SpMVError::INVALID_ARGUMENT);
    return result;
  }
  ...
  try {
    ...
    if (!dangling_nodes.empty()) {
      d_dangling_nodes.copyFromHost(dangling_nodes.data(),
                                    dangling_nodes.size());
    }
    ...
    const int dangling_blocks =
        dangling_nodes.empty()
            ? 0
            : static_cast<int>((dangling_nodes.size() + block_size - 1) /
                               block_size);
    ...
    for (int iter = 0; iter < config->max_iterations; iter++) {
      ...
      SpMVResult spmv_result =
          spmv_csr(adj_matrix, d_ranks_old.get(), d_ranks_new.get(),
...
void pagerank_top_k(const PageRankResult* result, int num_nodes, int k,
  ...
  std::partial_sort(
      nodes.begin(), nodes.begin() + actual_k, nodes.end(),
