...
TEST_F(BandwidthPropertyTest, MetricsValidity) {
    for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
        ...
        EXPECT_GE(bw.efficiency, 0.0f) << "Efficiency should be non-negative at iteration " << iter;

        EXPECT_LE(bw.efficiency, 1.0f) << "Efficiency should not exceed 1.0 at iteration " << iter;
