...
TEST(CommonTest, ErrorStringConversion) {
  ...
  EXPECT_STREQ(spmv_error_string(SpMVError::INVALID_DIMENSION),
               "Invalid matrix/vector dimension");
  EXPECT_STREQ(spmv_error_string(SpMVError::CUDA_MALLOC),
               "CUDA memory allocation failed");
  EXPECT_STREQ(spmv_error_string(SpMVError::CUDA_MEMCPY),
               "CUDA memory copy failed");
  EXPECT_STREQ(spmv_error_string(SpMVError::KERNEL_LAUNCH),
               "CUDA kernel launch failed");
  EXPECT_STREQ(spmv_error_string(SpMVError::INVALID_FORMAT),
  ...
  EXPECT_STREQ(spmv_error_string(SpMVError::INVALID_ARGUMENT),
               "Invalid argument");
}
