...
TEST_F(CSRPropertyTest, SerializationRoundTrip) {
  ...
  for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
    ...
    if (csr_original->nnz > 0) {
      EXPECT_TRUE(floatArraysEqual(csr_original->values, csr_loaded->values,
                                   csr_original->nnz));
      EXPECT_TRUE(intArraysEqual(csr_original->col_indices,
                                 csr_loaded->col_indices, csr_original->nnz));
    }
...
TEST(CSRUnitTest, HostMutationInvalidatesDeviceMirror) {
  ...
  CSRMatrix* csr = csr_create(0, 0, 0);
  ASSERT_EQ(csr_from_dense(csr, dense_a.data(), 3, 3),
  ...
  ASSERT_EQ(csr_from_dense(csr, dense_b.data(), 3, 3),
            static_cast<int>(SpMVError::SUCCESS));
...
TEST(CSRUnitTest, ValidateNullMatrix) { EXPECT_FALSE(csr_validate(nullptr)); }
