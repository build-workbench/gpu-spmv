...
TEST_F(ELLPropertyTest, DenseToSparseRoundTrip) {
  for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
    ...
    EXPECT_TRUE(floatArraysEqual(dense_original.data(),
                                 dense_reconstructed.data(), rows * cols))
...
TEST_F(ELLPropertyTest, SerializationRoundTrip) {
  ...
  for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
    ...
    size_t size = static_cast<size_t>(ell_original->num_rows) *
                  ell_original->max_nnz_per_row;
    if (size > 0) {
      EXPECT_TRUE(
          floatArraysEqual(ell_original->values, ell_loaded->values, size));
...
TEST(ELLUnitTest, HostMutationInvalidatesDeviceMirror) {
  ...
  ELLMatrix* ell = ell_create(0, 0, 0);
  ASSERT_EQ(ell_from_dense(ell, dense_a.data(), 3, 3),
  ...
  ASSERT_EQ(ell_from_dense(ell, dense_b.data(), 3, 3),
            static_cast<int>(SpMVError::SUCCESS));
...
TEST(ELLUnitTest, ValidateNullMatrix) { EXPECT_FALSE(ell_validate(nullptr)); }
