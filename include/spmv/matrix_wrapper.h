...
class CSRMatrixWrapper {
 public:
   ...
   */
  CSRMatrixWrapper(int rows, int cols, int nnz)
      : mat_(csr_create(rows, cols, nnz)) {}
   ...
   */
  CSRMatrixWrapper(CSRMatrixWrapper&& other) noexcept : mat_(other.mat_) {
    other.mat_ = nullptr;
  }
  ...
  int from_dense(const float* dense, int rows, int cols) {
    if (!mat_) {
      ...
      if (!mat_) return static_cast<int>(SpMVError::OUT_OF_MEMORY);
    }
  ...
  int to_dense(float* dense) const {
    if (!mat_) return static_cast<int>(SpMVError::INVALID_ARGUMENT);
...
class ELLMatrixWrapper {
 public:
   ...
   * @brief Move constructor.
   */
  ELLMatrixWrapper(ELLMatrixWrapper&& other) noexcept : mat_(other.mat_) {
    other.mat_ = nullptr;
  }
  ...
  int from_csr(const CSRMatrix* csr) {
    if (!mat_ || !csr) return static_cast<int>(SpMVError::INVALID_ARGUMENT);
