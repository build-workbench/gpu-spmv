...
class RandomGenerator {
   public:
    ...
    bool randBool(float probability = 0.5f) { return randFloat(0.0f, 1.0f) < probability; }
...
// 生成随机稠密矩阵
inline std::vector<float> generateRandomDenseMatrix(int rows, int cols, float density,
                                                    RandomGenerator& rng, float min_val = -10.0f,
                                                    float max_val = 10.0f) {
...
// 生成随机向量
inline std::vector<float> generateRandomVector(int size, RandomGenerator& rng,
...
// 比较两个浮点数组是否相等
inline bool floatArraysEqual(const float* a, const float* b, int size, float abs_tol = 1e-6f,
                             float rel_tol = 1e-6f) {
    for (int i = 0; i < size; i++) {
